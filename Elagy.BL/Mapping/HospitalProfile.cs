        ...
        public HospitalProfile()
        {
            ...
            CreateMap<DoctorCreateDto, Doctor>()
                  .ForMember(dest => dest.Id, opt => opt.Ignore()) // ID generated by Identity
                                                                   // ADJUSTMENT: Now explicitly set to true/Active
                  ...
                  .ForMember(dest => dest.Status, opt => opt.MapFrom(src => Status.Active)) // ADJUSTMENT: Doctor starts as Active
                  ...
                  // MedicalLicenseNumberId is typically an ID for an uploaded document, not direct DTO field
                  .ForMember(dest => dest.MedicalLicenseNumberId, opt => opt.Ignore())
                  .ForMember(dest => dest.Governorate, opt => opt.Ignore()) // Navigation property ignored for creation
                                                                            // Clarification: This is redundant because Governorate is already ignored. AutoMapper won't
                                                                            // try to map into a navigation property it's already ignoring. Harmless.
                  .ForMember(dest => dest.Governorate.Country, opt => opt.Ignore())
            ...
            CreateMap<DoctorUpdateDto, Doctor>()
                // ADJUSTMENT: REMOVED .ForMember(dest => dest.Email, opt => opt.Ignore())
                // Now Email will be mapped by convention, allowing it to be updated.
                ...
                .ForMember(dest => dest.Governorate, opt => opt.Ignore()) // Navigation property ignored
                                                                          // Clarification: This is redundant, same as above. Harmless.
                .ForMember(dest => dest.Governorate.Country, opt => opt.Ignore())
                .ForMember(dest => dest.HospitalSpecialty, opt => opt.Ignore()) // Navigation property ignored
                .ForMember(dest => dest.Schedules, opt => opt.Ignore()); // Navigation property ignored
